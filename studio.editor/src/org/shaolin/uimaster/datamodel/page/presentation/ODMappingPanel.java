...
public class ODMappingPanel extends Composite {
	 ...
	 * @param style
	 */
	public ODMappingPanel(Composite parent, int style) {
	...
	public void init(UIPageType model) {
		...
		varDefinition.init(this.editingDomain, model.getODMapping(), 
				model.getODMapping().getDataEntity());
				...
				for (EObject obj: eobjects) {
					if (obj instanceof UIPageType) {
				    	...
				    	if (page.getIn() == null) {
				    		...
				    		pageIn.getServerOperation().setExpressionString("{/n/n}");
				    	...
				    	if (page.getOut().size() == 0) {
							...
							defaultOut.getServerOperation().setExpressionString("{/n/n}");
