 ...
 * @generated
 */
public class VarMappingPropertySection extends AbstractModelerPropertySection implements
		IPropertySourceProvider {

	private VarMappingPropertyUI ui = null;
	
	public void createControls(final Composite parent,
	            TabbedPropertySheetPage aTabbedPropertySheetPage) {
	        super.createControls(parent, aTabbedPropertySheetPage);
	        
	        ui = new VarMappingPropertyUI(parent, SWT.NONE);
	}
	...
	public void setInput(IWorkbenchPart part, ISelection selection) {
		...
		if (ui != null) {
			OutType out = (OutType)this.getEObject();
			EList<NameExpressionType> mappings = out.getNext().getOutDataMappingToNode();
			WebNodeType node = (WebNodeType)out.eContainer();
			WebChunkType chuck = (WebChunkType)node.eContainer();
			WebNodeType nextNode = getNode(chuck, out.getNext().getDestNode());
			
			if (out instanceof DisplayOutType || out instanceof ConditionalOutType) {
				if (node instanceof StartNodeType) {
					ui.init(out, mappings, ((StartNodeType)node).getVariables(), 
							nextNode.getVariables());
				} else if (node instanceof LogicNodeType) {
					ui.init(out, mappings, ((LogicNodeType)node).getVariables(), 
							nextNode.getVariables());
				} else if (node instanceof PageNodeType) {
					ui.init(out, mappings, ((PageNodeType)node).getVariables(), 
							nextNode.getVariables());
				} else if (node instanceof FlowNodeType) {
					ui.init(out, mappings, ((FlowNodeType)node).getVariables(), 
							nextNode.getVariables());
				}
			}
		}
	...
	private WebNodeType getNode(WebChunkType chuck, String nodeName) {
		EList<WebNodeType> nodes = chuck.getWebNode();
